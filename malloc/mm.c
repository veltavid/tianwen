...
#define get_fbin(i) (mem_start_brk-init_size+8*i)
#define get_next(p) (*(unsigned long long*)(p+8))
...
int mm_init(void)
{
	...
	mem_sbrk(init_size);
	for(i=0;i<7;i++)
	set(mem_start_brk+8*i,0);//fastbin 0x10-0x40
	//set(mem_start_brk+8*7,0);//unsorted bin
	mem_start_brk+=init_size;
...
void *mm_malloc(size_t size)
{
    ...
    if(newsize<=0x40)
    {
    	...
    	bin=get_fbin(idx);
    	result=*(unsigned long long*)(bin);
    	if(result && idx==get_f_idx(get_size(result)))
    	{
    		set_4(result+newsize,newsize);
    		*(unsigned long long*)bin=get_next(result);
	    	return result+8;
	    }
    }
   	for(result=mem_brk;;)
